...
import org.openhab.binding.hdl.internal.HdlBindingConstants;
...
public class HdlHandler extends BaseThingHandler implements DeviceStatusListener {
    ...
    private final Logger logger = LoggerFactory.getLogger(HdlHandler.class);
            ...
            switch (getThing().getThingTypeUID().getAsString()) {
                ...
                case "hdl:MS12_2C":
                    ...
                    logger.debug("For Thing Type: {} with device id: {} with Refresh Interval: {} command is sent.",
                            new Object[] { getThing().getThingTypeUID().getAsString(), deviceID, refreshRate });
                    break;
                case "hdl:MPL8_48_FH":
    ...
    public void onDeviceStateChanged(ThingUID bridge, Device device) {
        if (device.getSerialNr().equals(hdldeviceSerial)) {
            if (device.isUpdated()) {
                logger.debug("Updating states of {} {} id: {}",
                        new Object[] { device.getType(), device.getSerialNr(), getThing().getUID() });
                ...
                logger.debug("No changes for {} {} id: {}",
                        new Object[] { device.getType(), device.getSerialNr(), getThing().getUID() });
            }
        }
    }
